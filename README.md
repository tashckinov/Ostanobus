...
Backend использует host network, потому что на целевом сервере системный nftables запрещает
форвардинг Docker bridge. Fastify при этом слушает только `127.0.0.1`, поэтому снаружи к нему
обращается Nginx, а не клиенты напрямую. Например, при `BACKEND_PORT=6767` upstream для Nginx —
`http://127.0.0.1:6767`.
...
- явный выбор линейного или кольцевого направления без автоматического замыкания;
- независимые сегменты между соседними остановками;
- автоматическая прокладка отдельного сегмента через OSRM, промежуточные точки и ручная линия;
- дорожные якоря остановок, проверка и фиксация готовых сегментов;
- точное расписание и интервалы движения отдельно для каждой остановки направления;
